...
class UserModuleTest:
    ...
    async def test(self):
        ...
        user_id = await api.execute( UserCreate(username="testuser") )
        user_id = await api.execute( UserCreate(username="testuser2") )
        user_id = await api.execute( UserCreate(username="testuser3") )
        user_id = await api.execute( UserCreate(username="testuser4") )
        user_id = await api.execute( UserCreate(username="testuser5") )
        assert user_id == 5
